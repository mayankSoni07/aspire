...
function DotsGroup({ count }: { count: number }) {
    return (
        <span className="inline-flex items-center gap-3">
            {Array.from({ length: count }).map((_, i) => (
                <span key={i} className="inline-block h-2 w-2 rounded-full bg-white align-middle" />
            ))}
        </span>
    );
}
            ...
            <div className="mt-6 sm:mt-10 text-lg sm:text-xl tracking-[0.2em] flex items-center">
                {showNumbers ? (
                    ...
                    <span className="inline-flex gap-6 items-center">
                        ...
                        <DotsGroup count={4} />
                        <DotsGroup count={4} />
                        <span className="ml-2">{groups[3]}</span>
                    </span>
                )}
            </div>
            <div className="mt-6 flex items-center justify-between text-xs sm:text-sm">
                <div>
                    <div className="uppercase opacity-80">Thru</div>
                    <div>
                        {String(card.expiryMonth).padStart(2, "0")}/{String(card.expiryYear).padStart(2, "0")}
                    </div>
                </div>
                <div>
                    <div className="uppercase opacity-80">CVV</div>
                    <div>{showNumbers ? "123" : "***"}</div>
                </div>
                <div className="flex items-center">
                    <Image src="/Visa Logo.svg" alt="VISA" width={54} height={22} />
                </div>
            </div>
            {card.isFrozen ? (
                <div className="absolute inset-0 rounded-2xl bg-black/20 backdrop-blur-[1px] flex items-center justify-center text-white font-medium">
                    Card Frozen
                </div>
            ) : null}
        </div>
